...
int32_t apply_diversity_functions_to_graph(const uint64_t i, struct measurement_configuration * const mcfg, struct measurement_structure_references * const sref, struct measurement_mutables * const mmut){
	...
	if(mcfg->threading.enable_iterative_distance_computation){
		...
		fprintf(mcfg->io.f_ptr, "%lu\t%li\t%li\t%li\t%s\t%li\t%.10e\t%lu\t%.10e\t%c", i+1, mmut->sentence.num, mmut->sentence.num_all, mmut->document.num, mcfg->io.w2v_path, sref->sorted_array_discarded_because_not_in_vector_database->num_elements, mmut->best_s, sref->g->num_nodes, mu_dist, '?'); // recomputing sigma dist would be expensive
		...
		*/
		...
		if(mcfg->io.enable_output_timing){
			fprintf(mcfg->io.f_timing_ptr, "%lu\t%li\t%li\t%li\t%s\t%li\t%.10e\t%lu", i+1, mmut->sentence.num, mmut->sentence.num_all, mmut->document.num, mcfg->io.w2v_path, sref->sorted_array_discarded_because_not_in_vector_database->num_elements, mmut->best_s, sref->g->num_nodes);
			if(time_ns_delta(NULL) != 0){goto time_ns_delta_failure;}
		}
		if(mcfg->io.enable_output_memory){
			fprintf(mcfg->io.f_memory_ptr, "%lu\t%li\t%li\t%li\t%s\t%li\t%.10e\t%lu", i+1, mmut->sentence.num, mmut->sentence.num_all, mmut->document.num, mcfg->io.w2v_path, sref->sorted_array_discarded_because_not_in_vector_database->num_elements, mmut->best_s, sref->g->num_nodes);
		}
		...
		fprintf(mcfg->io.f_ptr, "%lu\t%li\t%li\t%li\t%s\t%li\t%.10e\t%lu\t%.10e\t%.10e", i+1, mmut->sentence.num, mmut->sentence.num_all, mmut->document.num, mcfg->io.w2v_path, sref->sorted_array_discarded_because_not_in_vector_database->num_elements, mmut->best_s, sref->g->num_nodes, mu_dist, sigma_dist);
