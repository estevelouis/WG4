...
int32_t jsonl_to_graph(const uint64_t i, const char * const filename, struct measurement_configuration * const mcfg, struct measurement_structure_references * const sref, struct measurement_mutables * const mmut){
	...
	while(!(jdi.file_is_done)){
        ...
        pthread_mutex_lock(&sref->g->mutex_nodes);
		if((mcfg->target_column != UD_MWE || found_at_least_one_mwe) && (mcfg->steps.document.enable_count_recompute_step && (((!mcfg->steps.document.use_log10) && mmut->document.num % mcfg->steps.document.recompute_step == 0) || (mcfg->steps.document.use_log10 && mmut->document.num >= mmut->document.count_target)) && sref->g->num_nodes > 1)){
